...
function hobbiesPage()
{
    ...
    if (isset($_POST['uploadImgSubmit'])) {
        $target_file = $target_dir_img . "hobby_" . $hobby_name . ".jpg";
        $imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));
        $extensions_arr = array("jpg", "jpeg", "png", "gif");

        if (in_array($imageFileType, $extensions_arr)) {
            if (move_uploaded_file($_FILES['hobbyImgUpload']['tmp_name'], $target_file)) {
                $hobbyimg = "profile_picture_" . $hobby_name . ".jpg";
            }
        }
    }
?>
    <contentsection>
        ...
        <img src="/views/public/images/<?= $hobbyimg ?>" alt="">
...
$hobby_name = isset($_POST['hobbies']) ? $_POST['hobbies'] : "default";
